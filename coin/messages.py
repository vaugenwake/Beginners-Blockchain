...
from coin.schemas.peer import Peer
from coin.schemas.block import Block
from coin.schemas.ping import Ping
from coin.schemas.transaction import Transaction


class PeersMessage(Schema):
    payload = fields.Nested(Peer)
...
def meta(ip, port, version="coin-0.1"):
    return {
        "client": version,
        "address": {
            "ip": ip,
